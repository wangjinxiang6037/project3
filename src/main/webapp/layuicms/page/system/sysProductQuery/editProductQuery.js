...
*/
            ...
            function initUserInfo() {
                ...
                var url = $tool.getContext()+'application/get';
                var req = {
                    id: id
                };

                $api.GetProductQuery(req, function (res) {
                    var data = res.data;
                    console.log(data)

                    $("[name='id']").val(data.id);
                    $("[name='productId']").val(data.productId);
                    $("[name='productDamageId']").val(data.productDamageId);
                    $("[name='productSize']").val(data.productSize);
                    $("[name='productNumber']").val(data.productNumber);
                    $("[name='productPass']").val(data.productPass);
                    $("[name='productFail']").val(data.productFail)
                    /*orgId = data.orgId;
             ...
             * 表单提交
             * */
            form.on("submit(editProductQuery)", function (data) {
                var queryArgs = $tool.getQueryParam();//获取查询参数
                var id = queryArgs['id'];
                console.log(data)
               ...
               /* for (var i = 0; i < roleIdList.length; i++) {
                ...
                $api.updateProductQuery(req, function (data) {
