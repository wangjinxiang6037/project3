layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            ...
            , url: $tool.getContext() + 'damage/list' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                  {type:'numbers',title:'序号',fixed: 'left'}
                , {field: 'id', title: 'id'}
                , {field: 'productId', title: '破损单号'}
                , {field: 'productType', title: '破损类型' }
                , {field: 'productSize', title: '破损尺寸'}
    ...
    function delDamage(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.DeleteProductDamage(req, function (data) {
  ...
  /*  function initPwd(id){
    ...
    function editDamage(id){
        var index = layui.layer.open({
            title: "编辑用户",
            type: 2,
            content: "editProductDamage.html?id="+id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
